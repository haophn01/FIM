...
def monitor_files():
    """Monitor files for changes and keep a history of hashes."""
    file_hash_history = {}  # {file_path: {'last_hash': str, 'history': [str, ...]}}
    ...
    while True:
        for file_path in files_to_monitor:
            ...
            if file_path not in file_hash_history:
                file_hash_history[file_path] = {'last_hash': current_hash, 'history': []}
                print(f"Started monitoring {file_path}")
            else:
                last_hash = file_hash_history[file_path]['last_hash']
                if current_hash != last_hash:
                    print(f"File changed: {file_path}")
                    print(f"Previous hash: {last_hash}")
                    print(f"Current hash:  {current_hash}")
                    file_hash_history[file_path]['history'].append(last_hash)
                    file_hash_history[file_path]['last_hash'] = current_hash
...
Challenge 4: Implement Logging with Log Rotation
...
Challenge 5: Track the File’s Previous Hash and Display Only Changes
